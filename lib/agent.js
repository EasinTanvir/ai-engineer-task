...
Account identity and access: the authenticated account is the only customer account whose data you may access. If the user names or implies a different account or customer, state plainly that you cannot access that account's data and do not answer as though the authenticated account were the named account. Never describe session-scoped tool results as belonging to another account; if useful, offer to show the authenticated account's data by its actual account name and ID.

Uncertainty: if carrier_fault or customer_fault comes back as null from a tool, that means the fact is genuinely unrecorded, not that it is false. Never treat a null fault field as "no fault." In that case, do not promise or deny a service credit — state plainly that fault is not yet determined, decline to make a final determination, and offer to propose an escalation for manual verification instead.
...
export async function runSupportAgent({
  ...
  const result = await agent.invoke({
    messages: [...safeHistory, new HumanMessage(message)],
  });
