...
export async function POST(request) {
  ...
  if (!accountId || !sessionId) {
    return NextResponse.json({ error: "Choose an account before starting a chat." }, { status: 401 });
  }
  const { turn, confirmedToken } = startConversationTurn(sessionId, message);
  try {
    const result = await runSupportAgent({ accountId, sessionId, turn, confirmedToken, message, history });
    return NextResponse.json(result);
  } catch (error) {
    console.error("Chat request failed", error);
    return NextResponse.json({ error: "The support agent could not complete this request." }, { status: 500 });
  }
}
